#!/bin/bash
# spawn-openshift.sh: called by `ant beta:worker poll --on-work` for each session.
# Creates an OpenShell sandbox as a pod on a remote OpenShift/Kubernetes cluster
# and runs the ant worker session inside it.
#
...
#   OPENSHELL_GATEWAY (defaults to openshift-cluster)
set -euo pipefail

GATEWAY="${OPENSHELL_GATEWAY:-openshift-cluster}"
